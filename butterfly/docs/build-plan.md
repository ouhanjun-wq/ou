...
> **最终目标（验收标准）**：用 **盖世小鸡 G7 Pro 手柄**（或其他蓝牙手柄）操控，蝴蝶从手上起飞，在 **AUTO 模式**下持续飞行 **≥ 30 秒**。能按手柄或文本指令完成**加速 / 减速、左转 / 右转、爬升 / 下降、掉头、降落**；松开摇杆后，自动保持姿态、航向和高度（高度波动 ≤ ±1 m）；机体无损，**连续成功 3 次**。
>
> 这份计划是总入口，其他文档分工如下：
...
| 2 | G7 Pro 手柄配对 + 无线链路 | 1 天 | 能解锁，失控保护正常 |
...
## ✈️ 飞行能力与手柄操控（盖世小鸡 G7 Pro / Xbox 布局）
...
### 1.2 遥控器 / 地面站（盖世小鸡 G7 Pro）

| 编号 | 名称 | 型号 | 关键规格 | 数量 | 用途 | 搜索关键词 | 参考价 (¥) |
|---|---|---|---|---|---|---|---|
| R1 | ★ 游戏手柄 | **盖世小鸡 GameSir G7 Pro**（三模：Xbox 有线 / PC 2.4G / 安卓蓝牙） | 用它的**蓝牙模式**连接；TMR 摇杆、霍尔扳机，按键为 Xbox 布局 | 1 | 操控 | `盖世小鸡 G7 Pro` | 300–450（已有就不用买） |
...
**机身主体推荐“碳纤维管主梁 + 3D 打印舵机座 + 薄托板”**，结构见 [组装指南 图 7](assembly-guide.md#机身主体材料)，机身目标重量 8–12 g。

**✅ 翅膀材料已经有了：仿生蝴蝶材料包**（翼面布 + 碳杆 + 翼根座 + 胶 + 螺丝刀），下面不用再买：

| 材料包里的东西 | 数量 | 用在哪 | 代替原清单的 |
|---|---|---|---|
| 翼面布：**PC31 / P31N 特技风筝布**（涤纶防撕布），已经印好一对翅膀（前翅 + 后翅） | 1 张 | 翼面 | A5 聚酯薄膜 |
| 碳纤维杆（黑色长杆） | 4 根 | 每侧 2 根：前缘杆 + 后翅杆 | A3、A4 碳杆 |
| 黑色 X 形件 | 2 | 左右翼根座：碳杆插在上面，再装到舵机臂上 | A6 里的翼根座 |
| 黑色 T 形件 | 2 | 连接件（用途以卖家的安装视频为准，一般用来固定后翅杆或连接舵机臂） | — |
| 白色小卡件 | 4 | 碳杆上的卡扣 / 接头（以卖家说明为准） | — |
| 白管胶水 | 1 | 布胶：把布粘在碳杆上 | 3M 77 喷胶 |
| 红盖小瓶 | 1 | 瞬间胶：碳杆插进翼根座后点胶 | 502 |
| H1.5 × 50 螺丝刀 | 1 | 拧翼根座上的顶丝 / 小螺丝 | — |

> ⚠️ 材料包到手后先做三件事：
>
> 1. **量碳杆**：直径、长度各多少（卡尺量），记下来。
> 2. **称重**：翼面布、4 根碳杆、翼根座分别称一下。我们的蝴蝶**两只翅膀加起来最好 ≤ 25 g**（见 §1.6 重量预算）。
> 3. **试装翼根座**：舵机臂能不能装上去。装不上就用 `cad/wing_root.scad` 改参数打印一个转接件。
>
> 材料包的翅膀如果比原方案大、重，舵机扑不快：把最高扑翼频率调低，例如 `set f_max 4`（Hz）、`save`，吊线测试时再慢慢加。

**还要买的机体材料：**

| 编号 | 名称 | 型号 / 规格 | 数量 | 用途 | 搜索关键词 | 参考价 (¥) |
|---|---|---|---|---|---|---|
| A1 | ★ 机身主梁 | **碳纤维方管 4 × 4 mm**（壁厚约 0.5 mm）或碳纤维圆管 Ø4 / Ø3 mm，长 500 mm | 1–2 | 机身龙骨，截取 130–150 mm 使用（约 1.5 g） | `碳纤维方管 4mm` | 10–20 |
| A2 | 电子托板 | **碳纤维板 0.5 mm**（约 100 × 50 mm）；或 1.5 mm 轻木板 | 1 | 承载 XIAO、陀螺仪、电池 | `碳纤维板 0.5mm` / `轻木板 1.5mm` | 10–15 |
| A6 | 3D 打印件 | **舵机座 ×1**（模型和 STL 在 [`cad/`](../cad/README.md)）。翼根座用材料包里的；材料包的翼根座装不上舵机臂时，再打印 `cad/wing_root.scad` 做转接。**PETG**，层高 0.2 mm，3 圈壁，20–30% 填充，不需要支撑 | 1 套 | 夹持舵机和主梁 | `PETG 3D打印代打` | 10–30 |
| A7 | 胶 | **5 分钟 AB 胶**（舵机座和主梁连接处）；布胶和瞬间胶材料包里已经有 | 1 | 粘接 | `AB胶 5分钟` | 10 |
| A8 | 紧固件 | M2 × 6 螺丝和螺母；2 mm 宽小扎带；细尼龙线或风筝线 | 各 1 包 | 固定舵机、缠绕加固 | `M2 螺丝 套装` | 10 |
| A9 | 绝缘胶带 | Kapton（聚酰亚胺）胶带 10 mm | 1 卷 | **碳纤维导电**，贴在电子件下面绝缘 | `Kapton 胶带` | 5–10 |
| A10 | 魔术贴 | 10 mm 宽 | 1 小段 | 调重心阶段临时固定电池 | — | 2 |
...
| 笔刀、钢尺、切割垫、剪刀 | 剪风筝布要用锋利的剪刀 | 必须 | 20–40 |
| H1.5 螺丝刀 | 拧翼根座顶丝 | ✅ 材料包里已有 | — |
| USB-C 数据线 | 要能传数据，不能只是充电线 | 必须 ×2 | 20 |
| 2S 平衡充电器 | — | 可选，只在电池需要维护时用 | 80–250 |
| 3D 打印机 | — | 可选（也可以找代打） | — |

**预算合计**：不含工具和手柄，约 **¥500–1050**（翅膀材料已经有了，比原来少约 ¥50–80）；工具约 ¥250–500；如果还没有 G7 Pro 手柄，再加 ¥300–450。
...
- **充电锁**：只要插着充电线，飞控就拒绝解锁（台架模式也不行），LED 以 1 秒亮、1 秒灭闪烁。拔掉充电线后必须重新解锁：手柄上先按 B 上锁，再长按 A。
...
- [ ] 再添加 **esp32_bluepad32** 开发板包（给地面站用，步骤见阶段 2 的 2.1）。
...
- [ ] **G7 Pro 配对**：
  ...
  2. 短按 Xbox 键开机。
  3. **长按底部的配对键**，直到指示灯循环闪烁。
  4. 地面站会自动连接，串口显示 `gamepad connected: ...`，手柄指示灯变为常亮。
  5. 以后开机会自动回连。想换手柄时，串口输入 `PAIR`，清除旧的配对记录。
- [ ] 在串口输入 `STATUS`，应该显示 `gamepad ... (battery xx%)`。逐个推摇杆，核对方向：
  - 右摇杆向右 → `roll` 为 +
  - 右摇杆**向后拉** → `pitch` 为 +（抬头）
  - 左摇杆向右 → `yaw` 为 +
  - 左摇杆向前推 → `climb` 为 +
...
- [ ] **失控保护测试**：扑翼时把手柄关机（长按 Xbox 键） → 0.5 秒内停止扑翼，翅膀回到滑翔姿态，LED 快闪。手柄重新开机连上后 → 自动恢复控制。
...
- [ ] 3D 打印**舵机座**：打印前先量舵机（[组装指南 图 10](assembly-guide.md#机身主体材料)），尺寸不对就改 `cad/servo_mount.scad` 开头的参数，用 OpenSCAD 重新导出 STL（方法见 [`cad/README.md`](../cad/README.md)）。
- [ ] **翅膀（用仿生蝴蝶材料包）**，按[组装指南 4.2](assembly-guide.md#42-做翅膀) 和图 9 的 6 步做：
  1. 沿印刷轮廓外留 5–8 mm **剪下翼面**，电烙铁沿布边烫一圈防脱丝。
  2. 碳杆**按前缘长度截好**，杆头打毛，先插进材料包的翼根座试装。
  3. 沿印刷的前缘线涂一条**布胶**，放上碳杆，留边翻过来包住碳杆，压平晾干；后翅杆同样粘。
  4. 修边；**左右两只翅膀的重量差要 ≤ 0.5 g**，前缘刚度也要一致。
  5. 翼根座装到舵机臂上（装不上就打印 `cad/wing_root.scad` 转接）。
...
| 手柄一直连不上（`gamepad: none`） | 模式开关不在蓝牙档；手柄正在回连之前的手机；地面站用了 ESP32-S3（不支持经典蓝牙） | 模式开关拨到蓝牙；关掉手机的蓝牙；串口输入 `PAIR` 后长按手柄配对键；确认地面站是**原版 ESP32** |
| 手柄连上了但方向不对或按键错位 | 手柄在非 Xbox 布局的模式下 | 看 `STATUS` 显示的手柄型号；按键表以 Xbox 布局为准；必要时反馈给我调整映射 |
