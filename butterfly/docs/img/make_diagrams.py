...
def fig_wing_structure():
    s = Svg(1200, 720, "图 8  翅膀结构（左侧一片，俯视）· 用仿生蝴蝶材料包",
            "翼面是材料包里印好的风筝布，碳杆和翼根座也来自材料包；前翅、后翅插在同一个翼根座上，由同一个舵机带动")
    film = "#FFF3D6"
    edge = "#C9A55A"
    ...
    for tx, ty in ((262, 282), (430, 312), (620, 312), (540, 520), (660, 520)):   # printed veins
        s.wire([(815, 250 if ty < 400 else 335), (tx, ty)], "#6C9BD2", 1.4, True)
    s.wire([(815, 320), (522, 498)], "#2F3437", 5)                      # hindwing spar (kit rod)
    s.wire([(815, 250), (430, 312)], "#7A8084", 2.5)                    # optional thin rod
    # root joint
    s.parts.append(f'<rect x="812" y="215" width="46" height="160" rx="8" fill="#9EC5E8" stroke="{C["boxline"]}" '
                   f'stroke-width="1.5"/>')
    s.text(835, 395, "翼根座（材料包）", 12, "middle", "700")
    ...
    s.text(532, 102, "半翼展：按材料包翼面实际量（剪之前先量）", 12.5, "middle", "600")
    s.wire([(210, 150), (210, 285)], C["mute"], 1.2)
    s.text(200, 222, "前翅宽", 12, "end", "600")
    s.text(200, 238, "按翼面实量", 11.5, "end", color=C["mute"])
    # numbered markers on the drawing
    for n, (mx, my) in enumerate(((420, 176), (660, 408), (560, 290), (330, 240), (600, 460), (835, 300)), 1):
        s.marker(mx, my, n)
    # legend column
    items = [
        ("前缘杆：材料包碳杆", "最粗、受力最大，沿印刷的前缘线从翼根通到翼尖"),
        ("后翅杆：材料包碳杆", "后翅的主杆，和前缘杆插在同一个翼根座上"),
        ("可选细杆：Ø1–1.5 mm 碳杆", "翼面扑动时抖得厉害，再沿主翅脉加一根"),
        ("翼面：材料包风筝布（PC31 涤纶防撕布）", "印好的翅脉只是花纹；布只在碳杆处上胶"),
        ("蓝色虚线：印刷的翅脉花纹", "不用每条都加杆，加多了会太重"),
        ("翼根座：材料包的 X 形件", "装不上舵机臂就打印 cad/wing_root 转接"),
    ]
    ...
    s.box(40, 480, 330, 200, "剖面 A：布和碳杆怎么粘", "")
    ...
    s.text(205, 630, "布胶只涂在碳杆这一条；前缘留边翻过来包住杆", 12, "middle", "600")
    s.text(205, 648, "（整片布都涂胶会变重、变硬）", 11.5, "middle", color=C["mute"])
    s.text(344, 568, "布", 11.5, "end", color=C["mute"])
    ...
    s.text(620, 675, "碳杆插到孔底", 11.5, "middle", color=C["mute"])
    s.text(840, 636, "① 杆头用砂纸打毛再插", 12)
    s.text(840, 656, "② 有顶丝就用 H1.5 螺丝刀拧紧", 12)
    s.text(840, 676, "③ 孔口点一滴瞬间胶（红盖那瓶）", 12)
    s.save("fig8-wing-structure.svg")


def fig_wing_steps():
    s = Svg(1200, 760, "图 9  翅膀制作 6 步（仿生蝴蝶材料包）", "每一只翅膀都按这 6 步做；左右两只最后要称重配对")
    film, edge, rod = "#FFF3D6", "#C9A55A", "#2F3437"
    panels = [
        ("剪下翼面", ["沿印刷轮廓外留 5–8 mm 剪下", "电烙铁沿边烫一圈，布边不会脱丝"]),
        ("截杆试装", ["按前缘长度截杆，杆头砂纸打毛", "先插进翼根座，摆在印刷的前缘线上试长度"]),
        ("布胶粘杆", ["沿前缘线涂一条布胶（白管），放上碳杆", "留边翻过来包住碳杆，压平，按说明晾干"]),
        ("修边", ["胶干后修掉多余的布边", "后翅杆同样粘好；翼面平整不起皱"]),
        ("称重配对", ["两只翅膀重量差 ≤ 0.5 g", "重的那只修掉一点边缘的布"]),
    ...
    for i, (title, lines) in enumerate(panels):
        ...
        rods = [((ox + 250, oy + 52), (ox + 22, oy + 26), 5), ((ox + 245, oy + 70), (ox + 150, oy + 125), 4)]
        if i == 0:   # template on paper
            s.parts.append(f'<rect x="{ox - 15}" y="{oy - 10}" width="300" height="180" fill="#F7F9FB" '
                           f'stroke="#B9C4C1"/>')
            for gx in range(0, 300, 12):
                s.wire([(ox - 15 + gx, oy - 10), (ox - 15 + gx, oy + 170)], "#DCE3E8", 0.8)
            s.parts.append(f'<path d="{outline}" fill="#6C9BD2" fill-opacity="0.35" stroke="none"/>')
            s.text(ox + 270, oy + 160, "风筝布", 12, "end", "700", C["mute"])
            ...
            for a, b, w in rods:
                s.wire([a, b], rod, w)
            for dx, dy in ((245, 52), (245, 70), (136, 40)):
                s.parts.append(f'<circle cx="{ox + dx}" cy="{oy + dy}" r="5" fill="{C["v5"]}"/>')
            s.text(ox + 120, oy + 160, "● = 插进翼根座 / 上胶位置", 12, "middle", color=C["v5"])
            ...
            s.text(ox + 135, oy + 168, "包边压平，从中间向外抹平", 12, "middle", color=C["sig"])
        elif i == 3:  # trim
            ...
            s.text(ox + 20, oy + 160, "✂ 红虚线 = 修边线", 12, color=C["bat"])
