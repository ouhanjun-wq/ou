...
**先看结构**：每侧一片翅膀，由前翅和后翅组成。两根碳杆（前缘杆、后翅杆）插在同一个翼根座上，由同一个舵机带动。翼面是材料包里印好的风筝布，布上印的翅脉只是花纹。
...
1. **剪下翼面**：沿印刷轮廓外留 5–8 mm 剪下。用电烙铁（或热切刀）沿布边快速烫一圈，涤纶布边会熔合，不会脱丝。
2. **截杆试装**：量出前翅前缘线和后翅主杆的长度，把碳杆截好（用笔刀绕杆划一圈再掰断，或者细齿锯锯断）。杆头用砂纸打毛，先插进材料包的 X 形翼根座，摆在印刷的前缘线上，确认长度合适。
3. **布胶粘杆**：翼面反面朝上平铺。沿前缘线涂一条细细的布胶（白管），放上碳杆，把留边翻过来包住碳杆，压平。后翅杆同样粘好。按胶水说明晾干（一般要几个小时，完全固化最好放一晚）。
4. **修边**：胶干后修掉多余的布边。翼面要平整，不要起皱。
5. ✅ **两只翅膀称重，相差 ≤ 0.5 g**。重的那只修掉一点边缘的布。
6. **装到舵机**（见 4.3）。

**做翅膀的注意事项**：

- ⚠️ **左右两只翅膀要镜像**：材料包里的一对翅膀本来就是左右对称印的，前缘杆都要在**靠机头的一侧**。
- ⚠️ **胶只涂在碳杆那一条**（见图 8 剖面 A）。整片布都涂胶会变重、变硬。
- ⚠️ 截碳杆、打磨时戴口罩，最好蘸水打磨：碳纤维粉尘刺激皮肤和呼吸道。
- ⚠️ 先称一下材料包的碳杆和布：两只翅膀加起来最好 **≤ 25 g**。翅膀比原方案大、重的话，最高扑翼频率调低一点（`set f_max 4`，再 `save`）。
- 扑动时翼面抖得厉害，可以沿一条主要的印刷翅脉再加一根 Ø1–1.5 mm 细碳杆。不要每条都加，太重。

### 4.3 翅膀连接舵机

- 两根碳杆插进材料包的 **X 形翼根座**：杆头打毛，插到孔底。有顶丝就用材料包里的 **H1.5 螺丝刀**拧紧，孔口再点一滴**瞬间胶**（红盖那瓶）。
- **翼根座装到舵机臂上**：以卖家的安装视频为准，一般用舵机臂的小螺丝固定（黑色 T 形件可能就是用来连接的）。
- **材料包的翼根座装不上你的舵机臂**：用本项目的翼根座模型 [`cad/wing_root.scad`](../cad/wing_root.scad) 改参数打印一个（把碳杆直径 `rods` 改成材料包碳杆实际量到的直径），方法见 [`cad/README.md`](../cad/README.md)。

  ![翼根座 3D 模型（备用）](../cad/img/wing_root_left.png)

- ⚠️ **左右别装反**：前缘杆必须在**前面**（机头方向）。
...
| 3D 打印 PETG | 2.6 g（舵机座） | ★★★ | ★★★★ | 需要打印机或代打 | ✅ 舵机座首选（PLA 更脆，摔了容易裂） |
...
2. **打印舵机座**：模型是 [`cad/servo_mount.scad`](../cad/servo_mount.scad)，现成的 STL 是 [`cad/stl/servo_mount.stl`](../cad/stl/servo_mount.stl)（约 2.6 g）。**先用游标卡尺量舵机**（见下面图 10），尺寸不一样就改参数重新导出。材料选 **PETG**，层高 0.2 mm，3 圈壁，20–30% 填充，前面板平贴热床，不需要支撑。详细说明见 [`cad/README.md`](../cad/README.md)。

   ![舵机座 3D 模型](../cad/img/servo_mount.png)

   ![图 10 打印前要量的尺寸](img/fig10-cad-params.svg)

3. **舵机装进舵机座**：舵机从前面插进方孔，输出轴朝前，每个舵机 2 颗 M2 × 6 mm 螺丝。
4. **舵机座装到主梁前端**：主梁打毛，涂 **5 分钟 AB 胶**，从面板前面穿进方孔、穿过套筒，这里受力最大。胶固化前从正前方看，两个舵机轴要和主梁垂直、左右对称。
5. **切电子托板**：0.5 mm 碳板切成约 60 × 18 mm。**先在上面贴一层 Kapton 胶带绝缘**，再贴电子件。
6. **托板固定到主梁**：用细线缠绕 5–6 圈，再滴 502 让胶渗透进去；或者用 2 mm 宽的小扎带固定。
7. **头、尾（可选）**：3D 打印薄壳，或者用轻木削出形状，只用来做外观和保护，越轻越好。
...
| 两个舵机 | 机身前部，左右对称 | M2 螺丝 + 3D 打印舵机座（`cad/servo_mount.stl`） | 转轴在同一条直线上 |
