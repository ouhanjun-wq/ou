...
> 这份指南用**现成模块 + 免焊面包板（或洞洞板）**搭出和 Seeker 原版 PCB（Sesame V2）**完全相同的电路**。
...
| **D3** | GPIO4 | **电压检测模块的 S 脚**（模块内部 30 kΩ / 7.5 kΩ 分压；也可以自己用 100 kΩ 到 VIN、22 kΩ 到 GND） | R7 100k / R6 22k | 电池电压检测 |
...
> ✅ **XIAO ESP32S3 Plus 和 Sense 的 D0–D10 引脚完全一样**（D0 = GPIO1 … D5 = GPIO6、D6 = GPIO43、D7 = GPIO44、D8–D10 = GPIO7–9，对照 Arduino-ESP32 官方的 `XIAO_ESP32S3_Plus/pins_arduino.h`），上表照接，固件引脚不用改。
>
> Plus 要注意的几点：
> - **没有摄像头和麦克风**。摄像头功能用卫星板（BOM 的 E16）。
> - 背面多出来的 **D11–D19 小焊盘（1.27 mm）不用接**，焊排针时别让焊锡流过去短路。
> - **把棒状天线插到 U.FL 座上**，不插天线 Wi-Fi 很差，micro-ROS 会断。
> - 板子背面的 BAT+ / BAT− 电池焊盘**不要接**，全部从 5V 脚经 1N5819 供电。
>
> 如果用的是 Sense：它的 microSD 卡槽也用 D8–D10，**不要插 SD 卡**。
...
```
3S 电池 ─XT30─ 保险丝 5A ─ 船型开关 ─┬─ VIN ──────────── 电压检测模块 S → D3
                                     └─ 降压模块 IN+ ── OUT+ 5.0–5.2 V = 5V_SV ─┬─ PCA9685 V+（舵机）+ 1000 µF
                                                                            ├─ LD14P 5V
                                                                            ├─ MAX98357A VIN
                                                                            ├─ WS2812 5V
...
1. **先量降压模块（最重要）**：降压模块**什么都不接**，输入接电池，万用表量输出。推荐的**固定输出灌胶模块**应该在 **5.0–5.2 V**；用可调模块（XL4015 之类）就拧电位器调到 **5.10 V**。确认后再往下做。
...
5. OUT+ 同时引一根 22–24 AWG 线到面包板（或洞洞板）的 **5V_SV** 电源轨，再从这里分到雷达、功放、灯、1N5819。
6. **1N5819**：带**白色条纹**的一端（阴极）朝 XIAO 的 5V 脚，另一端接 5V_SV。
7. **电压检测模块**：模块的输入端子（VCC / GND）接 VIN 和 GND；输出针 **S → XIAO D3**、**− → GND**，**+ 不接**。不想买模块也可以自己搭：VIN → 100 kΩ → 中点 → 22 kΩ → GND，中点 → D3。
...
**方案一：免焊面包板（推荐先用这个跑通）**。XIAO（焊好排针）插在学习套件的 400 孔面包板中间，面包板两侧的孔当 3V3 和 GND 母线；PCA9685、BNO085、电压检测模块、雷达、OLED、功放都用**杜邦线**连到对应的孔。唯一要焊的是 1N5819：直接焊在 5V_SV 线和一根到 XIAO 5V 的杜邦线之间，套热缩管。走路会震，跑通以后在杜邦线接头上点一点热熔胶。

**方案二：洞洞板（长期用更可靠）**。一块 5 × 7 cm 洞洞板装：XIAO（插在排母上）、BNO085（插在排母上，保证水平）、1N5819、10 kΩ OE 上拉、330 Ω、100 µF，再引出几组排针：
...
| 11 | **甲板**：船型开关卡进甲板**后面**的方孔；PCA9685、面包板（或洞洞板）用尼龙柱 / 双面胶固定在孔阵上；甲板用 4 颗 M3 螺丝拧在铜柱上 | BNO085 的 X 箭头朝前（甲板竖板那边 = 前） |
| 12 | **雷达**：用 4 根 **M3 × 20 尼龙柱**把雷达架在甲板中间（柱子拧在孔阵上，雷达底座用双面泡棉胶或扎带固定），下面的空间放模块。**雷达的扫描面必须高于前面所有东西**（包括竖板和摄像头板） | 从侧面看，雷达镜头那一圈是整机最高的 |
| 13 | **摄像头板（可选）**：XIAO ESP32S3 Sense（或 ESP32-CAM）用扎带绑在甲板前沿的竖板上，镜头朝前；主板（Plus）装在面包板 / 洞洞板上 | |
...
| 2 | 拔掉 XIAO 和所有舵机线，只开电池开关 | 5V_SV = 5.0–5.2 V；D3 = 电池电压 × 0.2（12.0 V 电池 → 2.4 V） |
