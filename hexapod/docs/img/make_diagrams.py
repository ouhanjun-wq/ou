...
def fig_power():
    ...
    s.part(510, 170, 60, 24, "100 kΩ")
    s.wire([(540, 194), (540, 240)], C["sig"], 2)
    s.dot(540, 215, C["sig"])
    s.wire([(540, 215), (490, 215)], C["sig"], 2)
    s.tag(490, 215, "XIAO D3（电池电压）", C["sig"], "end")
    s.part(510, 240, 60, 24, "22 kΩ")
    s.wire([(540, 264), (540, 290)], C["gnd"], 2)
    s.tag(530, 290, "GND", C["gnd"], "end")
    # buck
    s.box(640, 190, 220, 110, "降压模块", "输入 7–24 V → 输出 5.1 V\n持续 ≥ 5 A（先调好再接！）")
    ...
    s.dot(700, 380, C["v5"])
    s.tag(40, 356, "5V_SV 舵机电源 5.1 V", C["v5"])
    ...
    # notes
    s.note(40, 700, 1, "降压模块先单独通电，用万用表调到 5.1 V，再接任何负载")
    ...
    s.note(600, 700, 4, "电池分压：VIN × 22 / 122 → 12.6 V 时 D3 只有 2.27 V")
