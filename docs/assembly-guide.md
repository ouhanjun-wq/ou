...
- **翼根座模型**：[`cad/wing_root.scad`](../cad/wing_root.scad)，现成的 STL 在 [`cad/stl/`](../cad/stl/)（`wing_root_left.stl` / `wing_root_right.stl`，左右各打一个，每个约 2.1 g）。打印前先量舵机臂的宽度和厚度（见图 10），改参数、打印、装配的方法都在 [`cad/README.md`](../cad/README.md)。

  ![翼根座 3D 模型](../cad/img/wing_root_left.png)

- **装到舵机臂上**：舵机臂插到插槽底，从上面拧一颗 M2 × 6 mm 螺丝穿过舵机臂最外面的孔，插槽口再点一滴 502。
- ⚠️ **左右别装反**：最粗的 Ø2 mm 前缘杆必须在**前面**（机头方向）。
