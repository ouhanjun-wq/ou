...
| 4 | ★ 电池 | **2S 7.4 V 150–200 mAh LiPo**（放电倍率 ≥ 30C） | 3 | 20–35 / 块 | 带平衡头。多备几块，飞得更久 |
...
| 8 | 电阻 | 200 kΩ 和 100 kΩ，1% 精度 | 各 2 | 2 | 电池电压分压（分压比 3.0） |
...
| 6 | 供电 | 充电宝（USB-C），或 1S LiPo 接 XIAO 的 BAT 焊盘 | 1 | 10–20 |
...
**预算合计**：不含工具约 **¥600–1200**，工具约 ¥300–700。
...
- [ ] 先把舵机臂**拆下来**，接好舵机电源（方案 A 直接接电池）和 470 µF 电容。
...
**✅ 出口条件**：IMU 四项方向检查全部正确；舵机方向和中位正确；台架扑翼 2 分钟不复位。
...
| 一扑翼就复位 | 舵机电流冲击导致电压跌落 | 检查 470 µF 电容和二极管；换放电倍率更高的电池；降压模块的输入端也加电容 |
...
| 升力不够 | 太重；扑翼频率或幅值不够 | 减重；提高 `f_max` / `amp_max`；把 `squareness` 调到 0.5；换满电电池 |
...
2. 解锁之前，手指要离开翅膀的运动范围。
3. 不在人群附近飞，不对着人脸飞。
4. 锂电池充电时有人看着，放在防爆袋里；飞完后检查电池有没有鼓包。
