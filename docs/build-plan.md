...
| A3 | 碳纤维实心杆 Ø2.0 mm | 长 500 mm | 4 | 翅膀前缘 | `碳纤维实心杆 2mm` | 10 |
| A4 | 碳纤维实心杆 Ø1.5 / Ø1.0 mm | 长 500 mm | 各 4–6 | 翅脉、翼尖 | `碳纤维实心杆 1mm` | 15 |
| A5 | 翼膜 | 聚酯薄膜（Mylar）12–15 µm，或 OPP 15–20 µm | A3 大小 × 4 张 | 翼面 | `聚酯薄膜 12微米` | 10–20 |
| A6 | 3D 打印件 | 舵机座 ×1、翼根座左右各 ×1（模型和 STL 在 [`cad/`](../cad/README.md)），头尾壳可选：**PETG**（比 PLA 韧，摔了不容易裂），层高 0.2 mm，3 圈壁，20–30% 填充，不需要支撑 | 1 套 | 夹持舵机和主梁 | `PETG 3D打印代打` | 20–60 |
| A7 | 胶 | 502 / CA 胶（低白化）+ 固化剂；3M 77 喷胶（贴膜用）；5 分钟 AB 胶（关键受力处） | 各 1 | 粘接 | `低白化502` `AB胶 5分钟` | 30–50 |
...
**预算合计**：不含工具和手柄，约 **¥550–1100**；工具约 ¥250–500；如果还没有 G7 Pro 手柄，再加 ¥300–450。
...
- [ ] 3D 打印舵机座和左右翼根座：打印前先量舵机和舵机臂（[组装指南 图 10](assembly-guide.md#机身主体材料)），尺寸不对就改 `cad/*.scad` 开头的参数，用 OpenSCAD 重新导出 STL（方法见 [`cad/README.md`](../cad/README.md)）。建议先打一套试装。
- [ ] **翅膀**：
