...
读手柄 / 串口指令 ─► 模式（关节 / XYZ / 自动动作 / 回放）─► 目标角度 q_t
                                                        │
      Servo 脉宽 D2–D8 ◄─ 两点标定换算 ◄─ 速度·加速度限制器 ◄──┘
                                   ▲
                   A3 舵机电压 ─► 急停检测
```
...
| $L_2$ | `GEO` 第 2 个 | J2 转轴到 J3 转轴 | $105\ \text{mm}$ |
| $L_3$ | `GEO` 第 3 个 | J3 转轴到 J4 转轴 | $100\ \text{mm}$ |
| $L_4$ | `GEO` 第 4 个 | J4 转轴到工具点（两指中间） | $120\ \text{mm}$ |
...
A3 上的电压经过 $10\,\text{k}\Omega + 10\,\text{k}\Omega$ 分压：

$$
V_{\text{舵机}} = \frac{\text{analogRead(A3)}}{1023}\times 5\ \text{V}\times 2
$$
...
**如果 J2 用套件自带的 MG996R**（省钱方案）：光是手臂自重，伸直时就要

$$
\tau_{J2,\ \text{无负载}} = 9.81\times(0.098 - 0.0325) \approx 0.64\ \text{N·m} \approx 6.6\ \text{kg·cm}
$$

而 MG996R 在 6 V 时只有约 $10\ \text{kg·cm}$，安全系数只有约 1.5。所以负载要 $\le 30\ \text{g}$，并尽量不要让手臂完全水平伸直。
