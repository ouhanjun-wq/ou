...
|---|---|---|---|---|
| [`electronics_tray.scad`](electronics_tray.scad) → [`stl/electronics_tray.stl`](stl/electronics_tray.stl) | 电控托板：洞洞板（ESP32）和 PCA9685 用立柱固定，降压模块、继电器、INA226、WAGO 端子用扎带固定 | 1 | 200 × 130 × 9 mm | 55–60 g |
...
| 零件 | 参数 | 含义 | 默认值 |
|---|---|---|---|
| 托板 | `perf` | 洞洞板：`[x, y, 长, 宽, 孔距x, 孔距y]` | 50 × 70 板，孔距 65 × 45 |
| 托板 | `pca` | PCA9685：同上 | 62.5 × 25.4，孔距 55.9 × 19.1 |
| 托板 | `buck` / `relay` / `ina` / `wago` | 扎带固定区：`[x, y, 长, 宽]` | 见文件 |
...
2. 洞洞板、PCA9685 用 **M2.5 × 6 自攻螺丝**拧在立柱上。
3. 20 A 降压模块、继电器、INA226、WAGO 端子：底面贴一层双面胶，再用 3 mm 扎带从托板下面的扎带孔穿上来绑紧。降压模块下面有通风孔，**散热片朝上**。
