...
//
// 所有电控模块装在这一块板上，板子再用 4 颗木螺丝固定在底板上（机械臂正后方，见图 6）。
// - 有固定孔的模块（洞洞板上的 ESP32、PCA9685）：用铜柱 / 打印的立柱 + M2.5 自攻螺丝
// - 固定孔不统一的模块（降压、继电器、INA226）：用扎带槽 + 双面胶
//
...
/* [模块]  位置 = 模块左下角 (x, y)；孔距 = 两个方向的孔中心距 */
// 洞洞板 50 x 70（上面焊 ESP32 排母、MP1584、SS14）
perf = [10, 10, 70, 50, 65, 45];          // [x, y, 长, 宽, 孔距x, 孔距y]
// PCA9685 常见板 62.5 x 25.4，孔距 55.9 x 19.1
pca = [95, 12, 62.5, 25.4, 55.9, 19.1];
// 扎带固定区： [x, y, 长, 宽]
buck = [10, 72, 66, 40];                  // 20 A 降压模块（带散热片）
relay = [95, 55, 50, 26];                 // 继电器模块
ina = [155, 55, 30, 22];                  // INA226 模块
wago = [95, 95, 90, 22];                  // WAGO 分线端子
...
difference() {
  ...
  for (m = [buck, relay, ina, wago]) { tie_slots(m); outline(m); }
  outline(perf);
  outline(pca);
  // 四角固定孔（带沉头）
